...
- `GET /v1/models` 透明代理
- 模型名称重定向：支持用户自定义下游 `model` 到上游 `model` 的精确映射
...
- 容器内 `:8080` 公共监听，提供 `/v1/messages`、`/v1/models`，同时也暴露 `/admin/*`
- 容器内 `:8081` 管理后台监听，只提供 `/admin/*`（便于绑定到内网地址做隔离）
- `docker-compose.yml` 默认只发布一个本机高位端口：`127.0.0.1:38080 -> 8080`
...
如需调整主机高位端口，可同时修改 `.env` 里的 `ANYMANAGER_PUBLIC_PORT`，默认值为 `38080`。
...
```bash
curl -X POST http://127.0.0.1:38080/admin/api/bootstrap \
...
```bash
curl http://127.0.0.1:38080/v1/messages \
...
可在管理后台“系统设置”里自定义精确匹配规则。命中后，仅会修改发往上游的请求体 `model` 字段，下游请求和返回内容保持不变。
...
那么下游继续请求 `claude-4-6-opus`，代理实际转发到上游时会自动改成 `claude-4-7-opus`。
...
如需通过 Nginx、Caddy 或其他反向代理对外提供服务，将公网域名反代到本机监听端口 `127.0.0.1:38080` 即可。
